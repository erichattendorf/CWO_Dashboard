...
def get_5min_asos():
    try:
        ...
        if response.status_code == 200:
            ...
            return None, None, None, None, "No data features found."
        else:
...
if latest_raw and latest_ts:
    try:
        ...
        with col_b:
            if age_minutes > 20: st.error(f"🚨 **COMM WARNING:** Last ASOS ping is **{int(age_minutes)} minutes old!** Check long-line.")
...
with top_col2:
    ...
    st.image("https://radar.weather.gov/ridge/standard/KBMX_loop.gif", caption="Live NWS BMX Radar Loop", use_container_width=True)
...
tab_svr, tab_wind, tab_ts, tab_precip = st.tabs(["🌪️ Severe Wx", "💨 Wind/Pressure", "🌩️ TS & Lightning", "🌧️ Precip/Hail/Vis"])

# Dictionary to hold all remarks in their designated letter slots
...
# Assemble the final string strictly following the alphabetical keys (A-W)
final_remarks = []
